...
**Publish (maintainer):** GitHub Actions only (`.github/workflows/publish.yml`) for OIDC + provenance. Current bump: **1.1.3**. Push, then Release `v1.1.3` or `workflow_dispatch`. Laptop `pnpm publish` is blocked (`provider: null` cannot mint provenance). Escape hatch: `FINETUNA_PUBLISH_LOCAL=1 pnpm publish` (no attestations). After CI: `npm view finetuna dist.attestations`.
